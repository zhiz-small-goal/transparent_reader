...
**后续 TODO：**  
- 无  
### [编号占位：例如 012] 2025-12-01 修复历史下一篇滚动恢复为顶部时继承上一文档
...
**目标：**  
- 历史“上一篇/下一篇”切换时，每个文档按各自滚动记录恢复  
- 当记录为顶部（0.0）时也显式滚动到顶，避免复用旧滚动状态

**触发原因：**  
- 从 B 回 A 再“下一篇”回 B 时，B 继承了 A 的滚动位置；原因是滚动比例为 0 时未执行恢复。

**涉及文件：**  
- src/app/MainWindow.cpp

**改动概览：**  
- MainWindow::applyScrollRatio：去除 ratio<=0 直接返回的早退，统一将比例夹到 [0,1] 并调用前端滚动恢复，确保顶部场景也会重置 scrollTop。

**关键点说明：**  
- 前端复用同一 WebView 时必须显式设置滚动位置，比例为 0 也要执行，以免上一文档的 scrollTop 残留。

**测试验证：**  
- [ ] 打开 A、B；B 保持顶部，A 滚动中部；从 B->A->B，确认 B 回到顶部  
- [ ] A、B 各有不同滚动比例，往返“上一篇/下一篇”均恢复各自位置  
- [ ] 新打开未滚动过的文档，加载后停在顶部  
- [ ] 右键翻页、锁定/解锁、托盘显隐等基础操作仍正常

**后续 TODO：**  
- 无

### [013] 2025-12-01 新增跳转顶部/底部入口
...
**目标：**  
- 在阅读器内提供快速返回顶部和跳转到底部的操作入口  
- 维持现有翻页与滚动恢复逻辑不变  

**触发原因：**  
- 需要在长文档中快速定位首尾，现有仅逐屏翻页无法满足  
...
**改动概览：**  
- `resources/web/main.js`：新增 `window.scrollToRatio(ratio)` 复用现有滚动容器探测，按比例滚动到顶/底  
- `src/app/MainWindow.h/.cpp`：新增 `scrollToTop/scrollToBottom` 槽，调用前端按比例滚动  
- `TitleBar`（`MainWindow.cpp` 内部类）：新增“顶部/底部”按钮，放在翻页按钮旁并接入新槽，尺寸样式随现有按钮统一放大  

**关键点说明：**  
- 按比例滚动仍优先命中实际可滚动容器（md-root/scrollingElement 等），未命中时回退窗口滚动  
- 保持滚动位置持久化逻辑不变，跳转后后台定时器仍会同步 SQLite 进度  

**测试验证：**  
- [ ] 打开长文档，点击“顶部”“底部”按钮可立即跳转到首尾  
- [ ] 翻页按钮、右键翻页在新增按钮后仍正常工作  
- [ ] 历史上一/下一篇切换后，各自滚动位置恢复正常  

**后续 TODO：**  
- [ ] 按需增加键盘快捷键（如 Home/End）映射到同一入口，确认不与 WebView 默认行为冲突  

### [014] 2025-12-01 修复历史上下篇共享滚动位置
...
**目标：**  
- 确保历史上一/下一篇切换时各文档按各自记录恢复滚动位置  
- 避免滚动比例为 0 时跳过恢复导致复用前一文档的滚动状态  

**触发原因：**  
- 点击“上一篇/下一篇”后，所有文档共享同一个滚动位置，顶部文档会继承前一篇的滚动状态  
...
**改动概览：**  
- `MainWindow.h`：将挂起滚动比例默认值改为 -1，用于明确区分“无记录”与“滚动到 0”  
- `MainWindow.cpp`：
  - loadFinished 回调中无条件应用当前文档的滚动记录，0 也会触发恢复
  - 打开已加载页面时，同样无条件调用 `applyScrollRatio`，避免继承上一文档的 scrollTop  

**关键点说明：**  
- 只调整滚动恢复条件，不改写滚动记录与节流逻辑；行为保持在各自文档范围内  

**测试验证：**  
- [ ] 打开 A、B；B 保持顶部，A 滚动中部；从 B->A->B，确认 B 回到顶部  
- [ ] A、B 各有不同滚动比例，往返“上一/下一篇”均恢复各自位置  
- [ ] 新打开未滚动过的文档，加载后停在顶部  
